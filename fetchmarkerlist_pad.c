/* fetchmarkerlist_pad.c, 1dec19, from: */
...
/* 1dec19, Handle padded indel data. */
...
int main (int argc, char *argv[]) {
  ...
  if (argc < 5) {
    ...
    printf("E.g. %s markers-fast ../data/generated_dataset_indels100.pad.h5 ../randomlists/h5_markers.100 ./fetchmarkerlist.out\n", argv[0]);
  ...
  if (strcmp(argv[1], "samples-fast") == 0) {    // "transposed" orientation
    ...
    for (k = 0; k < datumsize; k++)
      nullval[k] = 'N';
    char alleles[markerTotal * datumsize];
    int s; // sample number 
	...
	if (markers[i] < 0)
	  for (k = 0; k < datumsize; k++) 
	    alleles[i+k] = nullval[k];
	else 
	  for (k = 0; k < datumsize; k++) 
	    alleles[i+k] = rdata[markers[i] + k];  /*segfault*/
    }
  }
    ...
    /* If marker "position" is -1, it's missing. Return a row of Ns. */
	...
	fprintf(outfile, "\n");
      ...
      else {	/* It's a valid marker position. Read and output the alleles. */
	start[0] = markers[i];
	status = H5Sselect_hyperslab(dataspace_id, H5S_SELECT_SET, start, stride, count, block);
	/* Read the hyperslab. */
	status = H5Dread (dataset_id, datumtype, memspace_id, dataspace_id, H5P_DEFAULT, rdata);
	...
	for (j = 0; j < horizTotal * datumsize; j = j + datumsize) {
	  // Output only the first null-terminated string, not the padding.
	  fprintf(outfile, "%s", &rdata[j]);
	  /* No trailing <Tab> at end of line. */
	  if (j < (horizTotal - 1) * datumsize)
	    fprintf(outfile, "\t");
	}
