 /* loadmatrix_tetra_lkp.c, 29jan19, from:
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 4) {
    ...
    printf("Example: %s 7 ../data/generated_dataset_tetraploid.matrix ../data/generated_dataset_tetraploid_lkp.h5\n", argv[0]);
    printf("<datasize> is 7, e.g. \"C/C/1/1\" with no trailing NULL\n");
    return 0;
  }

// Hardcode it for tetraploid case:
#define PLOIDY 4  
  ...
  if (datumsize >  0 && datumsize <= 12  ) {
    datumtype = H5Tcreate(H5T_STRING,datumsize);
  } else {
    printf("Invalid <datasize>.\n");	
    printf("Must be a numeric value between 1 and 12.\n");
    return 1;
  } 
...
printf("Here!\n");
  ...
  infile = fopen(infilename, "r");
  int markernum = 0;
  while (fgets (row, 2000000, infile) != NULL)
    markernum++;
  ...
  char *dset_data = malloc(100000 * datumsize); // ?? SampleCount * datumsize?

  // Read the input matrix file into dset_data[].
  rownum = 0;
  infile = fopen(infilename, "r");
  ...
  while (fgets (row, 2000000, infile)) {
    /* insert = ""; // NO! Trashes the malloc size to huge. */
    insert[0] = '\0';
    // First token (genotype), strtok_r(row
    token = strtok_r(row, "\t", saveptr);
    outndx = 0;
    int i = 0;  // index of each character in this token
    // Check each tok of the token for an insert, and extract it.
    // Note: Need work here if this marker has more than one insert sequence.
    // First tok (allele call), strtok_r(token,
    tok = strtok_r(token, "/|", saveptr2);
    if (strlen(tok) > 1) {
      strcpy(insert, tok);
      /* dset_data[i] = 128;  // Replace the insert sequence with the first high-ASCII character? No, use "1". */
      dset_data[i] = '1'; i++;
    }
    else 
      dset_data[i] = tok[0]; i++;
    dset_data[i] = '/'; i++; 
    // Read the rest of the allele-call tok's, strtok_r(NULL,..saveptr2
    for (j=1; j<PLOIDY; j++) {
    ...
    /* Read the rest of the genotype tokens, strtok_r(NULL,..saveptr, into dset_data[]. */
    while ((token = strtok_r(NULL, "\t\n\r", saveptr))) {
      outndx++;
      i = 0;
      tok = strtok_r(token, "/|", saveptr2); 
      if (strlen(tok) > 1) {
	dset_data[(outndx * datumsize) + i++] = '1';
	strcpy(insert, tok);
      }
      else
	dset_data[(outndx * datumsize) + i++] = tok[0];
      dset_data[(outndx * datumsize) + i++] = '/';
      // Read the rest of the allele-call tok's, strtok_r(NULL,..saveptr2
      for (j=1; j<PLOIDY; j++) {
	tok = strtok_r(NULL, "/|\t", saveptr2);
	if (strlen(tok) > 1) {
 ...
 /* Create the initial hyperslab dimensions */
  ...
  char *row2 = malloc(2000000);
  FILE *infile2 = fopen (infilename, "r");
  rownum = 0;
  int batchcounter = 0;
  // Read each row of the input file into batchdata[][].
  while (fgets (row2, 2000000, infile2)) {
    i = 0;
    outndx = 0;
    // Read in the first token, column 0.
    token = strtok_r(row2, "\t", saveptr);
    tok = strtok_r(token, "/|", saveptr2); 
    // If it's an insert, replace with "1".
    if (strlen(tok) > 1) 
      tok = "1";
    batch_data[i][batchcounter] = tok[0]; i++;
    batch_data[i][batchcounter] = '/'; i++; 
	...
	tok = "1";
      batch_data[i][batchcounter] = tok[0]; i++;
      if (j < PLOIDY - 1)  // no trailing '/'
	batch_data[i][batchcounter] = '/'; i++;
    }
    outndx++;
    /* Read the rest of this input line into batch_data[]. */
    while (token = strtok_r(NULL, "\t\n\r", saveptr)) {
      i = 0;
      tok = strtok_r(token, "/|", saveptr2); 
      if (strlen(tok) > 1) 
	tok = "1";
      batch_data[(outndx * datumsize) + i][batchcounter] = tok[0]; i++;
      batch_data[(outndx * datumsize) + i][batchcounter] = '/'; i++;
      for (j=1; j<PLOIDY; j++) {
	tok = strtok_r(NULL, "/|", saveptr2);
	if (strlen(tok) > 1) 
	  tok = "1";
