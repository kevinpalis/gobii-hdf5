...
int main (int argc, char *argv[]) {
  ...
  if (strcmp(argv[1], "samples-fast") == 0) {    // "transposed" orientation
    ...
    tdimsmem[0] = markerCount;
    tdimsmem[1] = 1;
    tmemspace_id = H5Screate_simple(2, tdimsmem, NULL);
    /* Specify the hyperslab, one vertical column. */
    tstart[0] = 0;
    tstride[0] = 1; tstride[1] = 1;
    tcount[0] = 1; tcount[1] = 1;
    tblock[0] = markerCount; tblock[1] = 1;
    char *tdata = malloc(markerCount * datumsize);  /* temp buffer for transposed output */

time_t laptime = time(NULL);
elapsed = laptime - starttime;
printf("Time = %i sec. Reading input and transposing to temp file\n", elapsed);
    /* Read the input file, one row (sample) at a time. */
    /* todo: Batch into 10K-marker batches. */
	...
	for (k = 0; k < datumsize; k++)
	  tdata[i*datumsize + k] = rdata[markers[i]*datumsize + k];
      /* Adjust the temporary file's hyperslab column. */
      tstart[1] = samp;
      status = H5Sselect_hyperslab(tdataspace_id, H5S_SELECT_SET, tstart, tstride, tcount, tblock);
      /* Write the column. */
      status = H5Dwrite(tdataset_id, datumtype, tmemspace_id, tdataspace_id, H5P_DEFAULT, tdata);
    }
...
laptime = time(NULL);
elapsed = laptime - starttime;
printf("Time = %i sec. Dumping the temp file to output\n", elapsed);
    ...
    free(tdata);
    tdata = malloc(sampleTotal * datumsize);  
