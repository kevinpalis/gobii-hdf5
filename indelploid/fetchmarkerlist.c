/* fetchmarkerlist.c, 29jan20, from: */
...
/* 29jan20, Debug samples-fast orientation. */
...
int main (int argc, char *argv[]) {
  ...
  if (argc < 5) {
    ...
    printf("E.g. %s markers-fast ../data/generated_dataset_snp.h5 ../randomlists/h5_markers.5m ./fetchmarkerlist.out\n", argv[0]);
  ...
  if (strcmp(argv[1], "samples-fast") == 0) {    // "transposed" orientation
    ...
    /* char *alleles = malloc(markerCount * datumsize); */
    int s; // sample number 
	...
	for (k = 0; k < datumsize; k++) 
	  /* alleles[i+k] = rdata[markers[i] + k];   */
	  fprintf(outfile, "%c", rdata[(markers[i]*datumsize) + k]);  
	if (i < (markerCount - 1) * datumsize)
	  fprintf(outfile, "\t");
      }
      fprintf(outfile, "\n");
    }
