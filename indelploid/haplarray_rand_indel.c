/* haplarray_rand_indel.c, 13apr2020, from: */
...
int main (int argc, char *argv[]) {
  ...
  if (argc < 3) {
    printf("Usage: %s h5file insertsfile markerlist samplelist\n", argv[0]);
    printf("E.g. %s ../data/generated_dataset_indels100.lkp.h5 ../data/generated_dataset_indels100.lkp.h5.inserts ../randomlists/SNPs_only/h5_markers.1000 ../randomlists/SNPs_only/h5_samples.1000\n", argv[0]);
  ...
  char *markerlist = argv[3];
  char *samplelist = argv[4];
  ...
  for (i = 0; i < markerCount; ++i) {
	...
	if (rdata[samples[samp]*datumsize + k] == '1') {
	  ...
	  if (rdata[samples[samp]*datumsize + k] != '\0')  // Chop off the trailing <null> character.
	    fprintf(outfile, "%c", rdata[samples[samp]*datumsize + k]);
      fprintf(outfile, "\t");
    }
    fprintf(outfile, "\n");
  }
