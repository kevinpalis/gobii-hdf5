...
/* Load one dataset in csv format into the specified HDF5 file. 
   Load both normal orientation (sites-fast) and transposed (taxa-fast).
   No header row. One header column, the marker name. Values comma-separated.
   HDF5 datasize is passed as command line argument.    
   Load time for NAM c6-10 was 1.8 hr for sites-fast plus 2.8 hr for taxa-fast.
*/
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 4) {
    ...
    printf("Example: %s 1 /local/data/NAM_H32/csv/c6-10.csv /local/data/NAM_H32/NAMc6-10.h5\n", argv[0]);
