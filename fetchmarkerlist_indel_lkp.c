...
/* int binsearch(int i, int lower, int upper, int target); */
/* int binsearch(int lower, int upper, int target); */
// Marker-numbers that are indels.  Loaded from *.h5.inserts file and used by binsearch().
/* int *markernums = (int *)malloc(5000000 * sizeof(int)); */
...
int main (int argc, char *argv[]) {
  ...
  if (argc < 5) {
    printf("Usage: %s <HDF5 file> <insertsfile> <queryfile> <output file>\n", argv[0]);
    printf("E.g. %s ../data/generated_dataset_indels100.lkp.h5 ../data/generated_dataset_indels100.lkp.h5.inserts ../randomlists/h5_markers.1000 fetchmarkerlist.out\n", argv[0]);
  ...
  /* Read and output the alleles for each marker. */
  for (i = 0; i < markerCount; i++) {
	...
	if (rdata[j + k] == '1') {
	  if (lookedup == false) {  // first encounter with this marker on this row of input file
	    // Look it up in the inserts arrays. markers[i] is the number of the current marker, its position in the allelematrix.
	    /* s = binsearch(0, 0, insertCount, markers[i]); */
/* printf("markernums[654368] = %i\n", markernums[654368]); */
/* 	    s = binsearch(0, insertCount, markers[i]); */
/* 	    printf("s = %i, seq = %s\n", s, seqs[s]); */
	    /* No, using linear search for now: */
	    s = 0;
...
/* /\* Binary search for a particular integer "target" in a sorted array "markernums[]". *\/ */
/* int binsearch(int lower, int upper, int target) { */
/*   int m = (lower + upper) / 2; */
/*   if (markernums[m] == target) { */
/* printf("markernums[m] = %i, target = %i, m = %i\n", markernums[m], target, m); */
/*     return m; */
/*   } */
/*   if (markernums[m] < target) { */
/*     lower = m; */
/* /\* printf("m = %i, markernums[m] = %i, lower = %i\n", m, markernums[m], lower); *\/ */
/*     binsearch (lower, upper, target); */
/*   } */
/*   if (markernums[m] > target) { */
/*     upper = m; */
/* printf("markernums[m] = %i, upper = %i\n", markernums[m], upper); */
/*     binsearch (lower, upper, target); */
/*   } */
/* } */
